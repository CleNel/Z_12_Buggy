...
void Echoed() {
  ...
  /*
  rel_speed=(distance-lastdistance)/(millis()-velocity);
...
void loop() {
  ...
  */
  ...
  if (abs(millis()-previousTime1) > 1000){
    rel_speed=(lastdistance-distance)*1000/(previousTime1 -millis());
    lastdistance=distance;
  ...
  */
  ...
  if (distance < 400){
    ...
    send("@" + String(velocity)+","+String("N/A") + "#");
  }
  //send("@" + String(velocity)+","+String(rel_speed+velocity) + "#");
